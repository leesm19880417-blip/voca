# -*- coding: utf-8 -*-
"""해커스 실전 1000제 2권 해설집 회차 렌더 (OCR용)
usage: python _hk2_render.py <T> [lc|rc]
  LC 해설 Q1 = 21 + 42*(T-1)   (42p/회차, 검증됨)
...
LC_A = os.path.join(D, '해커스 토익 실전 1000제 LC 2 해설집.pdf')
RC_A = os.path.join(D, '해커스 토익 실전 1000제 RC 2 해설집.pdf')
...
def render(T, which='lc'):
    if which == 'lc':
        d = fitz.open(LC_A); s = lc_start(T); n = 42; out = os.path.join(BASE, '_hk2lc%d' % T); pre = 'la'
    else:
        if T not in RC_START:
            raise SystemExit('RC Test%d 시작페이지 미확정 — 프로브 후 RC_START에 추가하세요' % T)
        d = fitz.open(RC_A); s = RC_START[T]; n = 56; out = os.path.join(BASE, '_hk2rc%d' % T); pre = 'ra'
